...
demandTPM = np.array([[0.2, 0.4, 0.1, 0.1, 0.2], [0.3, 0.3, 0.2, 0.1, 0.1], [0.2, 0.2, 0.3, 0.1, 0.2], [0.1, 0.2, 0.4, 0.2, 0.1], [0.25, 0.15, 0.2, 0.1, 0.3]])
priceTPM = np.array([[0.3, 0.2, 0.15, 0.1, 0.25], [0.1, 0.2, 0.3, 0.2, 0.2], [0.25, 0.35, 0.1, 0.15, .15], [0.1, 0.4, 0.1, 0.1, .3], [0.1, 0.2, 0.4, 0.2, 0.1]])
...
while(m<maxIterations):
    ...
    if h == horizons:
        QMatrix_current[h, s, a] = (1 - a_n(m)) * QMatrix_current[h, s, a] + a_n(m) * (current_cost)
    else:
        QMatrix_current[h, s, a] = (1 - a_n(m)) * QMatrix_current[h, s, a] + a_n(m) * (current_cost + np.amin(QMatrix_prev[h + 1, snext, 0:len(A)]))
